using UnityEngine;
using System.Collections;
using FboPingPong = Utils.FboPingPong;

[RequireComponent (typeof(Camera))]
public class InkSim : MonoBehaviour {

	[SerializeField] float omega = 1.45f;

	// lbm fluid sim
	[SerializeField] Shader initShader;
	[SerializeField] Shader compShader;
	[SerializeField] Shader stepShader;
	[SerializeField] Shader copyShader;
	[SerializeField] Shader bounceBackShader;
	[SerializeField] Shader showShader;
	[SerializeField] Shader paintShader;
	[SerializeField] Shader fillShader;

	// paper sim
	[SerializeField] Shader surfShader;
	[SerializeField] Shader flowShader;
	[SerializeField] Shader fixShader;

	[SerializeField] Shader debugShader;

	Material initMat;
	Material compMat;
	Material stepMat;
	Material copyMat;
	Material bounceBackMat;
	Material paintMat;

	Material surfMat;
	Material flowMat;
	Material fixtureMat;
	Material fillMat;

	Material showMat;
	Material debugMat;

	RenderTexture rt1;
	RenderTexture rt2;
	RenderTexture rt3;
	RenderTexture rt1c;
	RenderTexture rt2c;
	RenderTexture rt3c;
	RenderTexture rtMask;

	Texture2D white;
	[SerializeField] Texture pinning;

	FboPingPong surface;
	FboPingPong flow;
	FboPingPong fixture;

	bool isDragging = false;

	void Start () {
		SetupRTs();
		SetupMats();

		// init 
		Graphics.Blit (null, rtMask, fillMat);
		Graphics.Blit (null, surface.GetReadTex(), fillMat);
		Graphics.Blit (null, flow.GetReadTex(), fillMat);
		Graphics.Blit (null, fixture.GetReadTex(), fillMat);

		white = new Texture2D(1, 1);
		white.SetPixel(0, 0, Color.white);
	}
	
	void Update () {

		if(Input.GetMouseButtonDown(0)) {
			isDragging = true;
		} else if(Input.GetMouseButtonUp(0)) {
			Graphics.Blit (null, rtMask, initMat);
			isDragging = false;
		}

		if(isDragging) {
			Graphics.Blit (null, rtMask, initMat);

			Vector3 mousePos = Input.mousePosition;
			float mx = mousePos.x / Screen.width;
			float my = mousePos.y / Screen.height;
			paintMat.SetFloat("_X", mx);
			paintMat.SetFloat("_Y", my);
			Graphics.Blit (null, rtMask, paintMat);
		}

		Graphics.Blit(null, rt1c, compMat, 0);
		Graphics.Blit(null, rt2c, compMat, 1);
		Graphics.Blit(null, rt3c, compMat, 2);

		Graphics.Blit(null, rt1, copyMat, 0);
		Graphics.Blit(null, rt2, copyMat, 1);
		Graphics.Blit(null, rt3, copyMat, 2);

		Graphics.Blit(null, rt1c, bounceBackMat, 0);
		Graphics.Blit(null, rt2c, bounceBackMat, 1);
		Graphics.Blit(null, rt3c, bounceBackMat, 2);

		Graphics.Blit(null, rt1, copyMat, 0);
		Graphics.Blit(null, rt2, copyMat, 1);
		Graphics.Blit(null, rt3, copyMat, 2);

		Graphics.Blit(null, rt1c, stepMat, 0);
		Graphics.Blit(null, rt1, copyMat, 0);

		Graphics.Blit(null, rt2c, stepMat, 1);
		Graphics.Blit(null, rt2, copyMat, 1);

		Graphics.Blit(null, rt3c, stepMat, 2);
		Graphics.Blit(null, rt3, copyMat, 2);

		compMat.SetFloat("_Omega", omega);
		compMat.SetFloat("_T", Time.timeSinceLevelLoad);
		stepMat.SetFloat("_T", Time.timeSinceLevelLoad);

		surfMat.SetColor("_Color", new Color(
			Mathf.PerlinNoise(0f, Time.time * 0.4f) * 0.5f,
			Mathf.PerlinNoise(0f, Time.time * 0.5f) * 0.5f,
			Mathf.PerlinNoise(0f, Time.time) * 0.5f,
			1f
		));
		surfMat.SetTexture("_SurfTex", surface.GetReadTex());
		Graphics.Blit (null, surface.GetWriteTex(), surfMat);
		surface.Swap();

		flowMat.SetTexture("_SurfTex", surface.GetReadTex());
		flowMat.SetTexture("_FlowTex", flow.GetReadTex());
		flowMat.SetTexture("_FixtureTex", fixture.GetReadTex());
		Graphics.Blit (null, flow.GetWriteTex(), flowMat);
		flow.Swap();

		fixtureMat.SetTexture("_SurfTex", surface.GetReadTex());
		fixtureMat.SetTexture("_FlowTex", flow.GetReadTex());
		fixtureMat.SetTexture("_FixtureTex", fixture.GetReadTex());
		Graphics.Blit (null, fixture.GetWriteTex(), fixtureMat);
		fixture.Swap();

	}

	void OnDestroy () {
		rt1.Release();
		rt2.Release();
		rt3.Release();
		rt1c.Release();
		rt2c.Release();
		rt3c.Release();
		rtMask.Release();
	}

	void OnGUI () {

		if(!Event.current.type.Equals(EventType.Repaint)) return;

		showMat.SetTexture("_SurfTex", surface.GetReadTex());
		showMat.SetTexture("_FlowTex", flow.GetReadTex());
		showMat.SetTexture("_FixtureTex", fixture.GetReadTex());
		Graphics.DrawTexture(new Rect(0, 0, Screen.width, Screen.height), white, showMat);

		int size = 48;
		var r00 = new Rect(0, size * 0, size, size);
		var r01 = new Rect(0, size * 1, size, size);
		var r02 = new Rect(0, size * 2, size, size);
		var r03 = new Rect(0, size * 3, size, size);
		Graphics.DrawTexture(r00, white, debugMat);
		Graphics.DrawTexture(r01, surface.GetReadTex());
		Graphics.DrawTexture(r02, flow.GetReadTex());
		Graphics.DrawTexture(r03, fixture.GetReadTex());

	}

	void SetupRTs () {
		rt1 = CreateRenderTexture(Screen.width, Screen.height);
		rt2 = CreateRenderTexture(Screen.width, Screen.height);
		rt3 = CreateRenderTexture(Screen.width, Screen.height);
		rt1c = CreateRenderTexture(Screen.width, Screen.height);
		rt2c = CreateRenderTexture(Screen.width, Screen.height);
		rt3c = CreateRenderTexture(Screen.width, Screen.height);
		rtMask = CreateRenderTexture(Screen.width, Screen.height);

		surface = new FboPingPong(Screen.width, Screen.height);
		flow = new FboPingPong(Screen.width, Screen.height);
		fixture = new FboPingPong(Screen.width, Screen.height);
	}

	void SetupMats () {
		initMat = new Material(initShader);
		copyMat = new Material(copyShader);
		compMat = new Material(compShader);
		stepMat = new Material(stepShader);
		bounceBackMat = new Material(bounceBackShader);
		showMat = new Material(showShader);
		paintMat = new Material(paintShader);
		surfMat = new Material(surfShader);
		flowMat = new Material(flowShader);
		fixtureMat = new Material(fixShader);
		fillMat = new Material(fillShader);
		debugMat = new Material(debugShader);

		copyMat.SetTexture("_Diffuse1", rt1c);
		copyMat.SetTexture("_Diffuse2", rt2c);
		copyMat.SetTexture("_Diffuse3", rt3c);

		compMat.SetTexture("_Diffuse1", rt1);
		compMat.SetTexture("_Diffuse2", rt2);
		compMat.SetTexture("_Diffuse3", rt3);
		compMat.SetTexture("_DisorderTex", pinning);
		compMat.SetFloat("_Omega", omega);
		compMat.SetFloat("_Dx", 1f / Screen.width);
		compMat.SetFloat("_Dy", 1f / Screen.height);

		bounceBackMat.SetTexture("_Diffuse1", rt1);
		bounceBackMat.SetTexture("_Diffuse2", rt2);
		bounceBackMat.SetTexture("_Diffuse3", rt3);
		bounceBackMat.SetTexture("_Mask", rtMask);

		stepMat.SetTexture("_Diffuse1", rt1);
		stepMat.SetTexture("_Diffuse2", rt2);
		stepMat.SetTexture("_Diffuse3", rt3);
		stepMat.SetFloat("_Dx", 1f / Screen.width);
		stepMat.SetFloat("_Dy", 1f / Screen.height);

		surfMat.SetTexture("_Mask", rtMask);
		surfMat.SetFloat("_Dx", 1f / Screen.width);
		surfMat.SetFloat("_Dy", 1f / Screen.height);

		flowMat.SetTexture("_Diffuse1", rt1c);
		flowMat.SetTexture("_Diffuse2", rt2c);
		flowMat.SetTexture("_Diffuse3", rt3c);
		flowMat.SetFloat("_Dx", 1f / Screen.width);
		flowMat.SetFloat("_Dy", 1f / Screen.height);

		fixtureMat.SetFloat("_Dx", 1f / Screen.width);
		fixtureMat.SetFloat("_Dy", 1f / Screen.height);

		debugMat.SetTexture("_Diffuse1", rt1c);
		debugMat.SetTexture("_Diffuse2", rt2c);
		debugMat.SetTexture("_Diffuse3", rt3c);
		debugMat.SetFloat("_Dx", 1f / Screen.width);
		debugMat.SetFloat("_Dy", 1f / Screen.height);
	}

	RenderTexture CreateRenderTexture (int width, int height) {
		RenderTexture rt = new RenderTexture(width, height, 0);
		rt.format = RenderTextureFormat.ARGBFloat;
		rt.wrapMode = TextureWrapMode.Repeat;
		rt.filterMode = FilterMode.Point;
		rt.Create();
		return rt;
	}

}
